...
function isInsideCircle(circle, point) {
  ...
  copyPoint.x -= circle.center.x;
  copyPoint.y -= circle.center.y;
  if (circle.radius ** 2 > copyPoint.x ** 2 + copyPoint.y ** 2) {
    return true;
  }
  return false;
}
...
function findFirstSingleChar(str) {
  ...
  let result = null;
  result = strArray.find((character, index, array) => {
    if (array.lastIndexOf(character) === index) {
      return true;
    }
    return false;
  });
